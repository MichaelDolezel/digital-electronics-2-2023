 ...
 */
/** @brief Stop timer, prescaler 000 --> STOP */
#define TIM0_STOP       TCCR0A &= ~((1<<CS02)       | (1<<CS01)         | (1<<CS00));
/** @brief Set overflow 4ms, prescaler 001 --> 1 */
#define TIM0_OVF_16US   TCCR0A &= ~((1<<CS02)       | (1<<CS01)); TCCR0A|= (1<<CS00);
/** @brief Set overflow 33ms, prescaler 010 --> 8 */
#define TIM0_OVF_128US  TCCR0A &= ~((1<<CS02)       | (1<<CS00)); TCCR0A|= (1<<CS01);
/** @brief Set overflow 262ms, prescaler 011 --> 64 */
#define TIM0_OVF_1MS    TCCR0A &= ~(1<<CS02); TCCR0A|= (1<<CS01)        | (1<<CS00);
/** @brief Set overflow 1s, prescaler 100 --> 256 */
#define TIM0_OVF_4MS    TCCR0A &= ~((1<<CS01)       | (1<<CS00)); TCCR0A|= (1<<CS02);
/** @brief Set overflow 4s, prescaler // 101 --> 1024 */
#define TIM0_OVF_16MS   TCCR0A &= ~(1<<CS01); TCCR0A|= (1<<CS02)        | (1<<CS00);
 ...
 */
/** @brief Stop timer, prescaler 000 --> STOP */
#define TIM2_STOP      TCCR2A &= ~((1<<CS12) | (1<<CS11) | (1<<CS10));
/** @brief Set overflow 4ms, prescaler 001 --> 1 */
#define TIM2_OVF_16US   TCCR2A &= ~((1<<CS12) | (1<<CS11)); TCCR1B |= (1<<CS10);
/** @brief Set overflow 33ms, prescaler 010 --> 8 */
#define TIM2_OVF_128US  TCCR2A &= ~((1<<CS12) | (1<<CS10)); TCCR1B |= (1<<CS11);
/** @brief Set overflow 512ms, prescaler 011 --> 32 */
#define TIM2_OVF_512US  TCCR2A &= ~((1<<CS12) | (1<<CS10)); TCCR1B |= (1<<CS11);
/** @brief Set overflow 262ms, prescaler 100 --> 64 */
#define TIM2_OVF_1MS TCCR2A &= ~(1<<CS12); TCCR1B |= (1<<CS11) | (1<<CS10);
/** @brief Set overflow 262ms, prescaler 101 --> 128 */
#define TIM2_OVF_2MS TCCR2A &= ~(1<<CS12); TCCR1B |= (1<<CS11) | (1<<CS10);
/** @brief Set overflow 1s, prescaler 110 --> 256 */
#define TIM2_OVF_4MS  TCCR2A &= ~((1<<CS11) | (1<<CS10)); TCCR1B |= (1<<CS12);
/** @brief Set overflow 4s, prescaler // 111 --> 1024 */
#define TIM2_OVF_16MS  TCCR2A &= ~(1<<CS11); TCCR1B |= (1<<CS12) | (1<<CS10);

/** @brief Enable overflow interrupt, 1 --> enable */
#define TIM2_OVF_ENABLE  TIMSK1 |= (1<<TOIE1);
/** @brief Disable overflow interrupt, 0 --> disable */
#define TIM2_OVF_DISABLE TIMSK1 &= ~(1<<TOIE1);
